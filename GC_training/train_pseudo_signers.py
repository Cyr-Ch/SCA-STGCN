"""
Training script for Pseudo-Signer Clustering Model on Google Cloud.

Usage:
    python train_pseudo_signers.py \
        --gcs-data gs://your-bucket/segments \
        --output-dir ./runs/pseudo_signers \
        --epochs 50 \
        --batch 64
"""
...
from train_base import train_model, setup_gcs_path


def main():
    parser = argparse.ArgumentParser(description="Train pseudo-signer clustering model on Google Cloud")
    ...
    parser.add_argument('--num-pseudo-signers', type=int, default=4, help='Number of pseudo-signer clusters')
    parser.add_argument('--signer-loss-weight', type=float, default=0.5, help='Weight for adversarial signer loss')
    ...
    # Output arguments
    parser.add_argument('--output-dir', type=str, default='./runs/pseudo_signers', help='Output directory for checkpoints')
    ...
    train_ds = NpzLandmarksDataset(
        ...
        preprocessed_dir=data_path if not data_path.startswith('gs://') else None,
        split='train',
    )
    ...
    model_config = {
        ...
        'num_pseudo_signers': args.num_pseudo_signers,
        'signer_loss_weight': args.signer_loss_weight,
        'use_supcon': False,
